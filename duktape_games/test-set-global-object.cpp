...
TEST_F(FooTest, test_set_global_object)
{
	duk_smart_ptr duk_instance(duk_create_heap_default(), [](duk_context* f) { duk_destroy_heap(f); });
	run_tests({ test_invalid_index,
		test_invalid_target,
		test_basic,
		test_noeval,
		test_regexp_literals,
		test_regexp_prototype_shared,
		test_set_after_thread_create,
		test_set_before_thread_create  });	
}
